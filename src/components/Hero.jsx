...
export default function Hero({ isMobile, reducedMotion }) {
  ...
  return (
    <section
      ...
      style={{ position: 'relative', minHeight: '100svh', overflow: 'hidden', display: 'flex', alignItems: isMobile ? 'flex-start' : 'center' }}
    >
      <div
        ref={bgRef}
        aria-hidden="true"
      ...
      <div
        ref={textRef}
        style={isMobile
          ? { padding: '38svh 24px 64px', width: '100%', position: 'relative', zIndex: 2 }
          : { padding: '110px 0 0 6vw', maxWidth: '52%', position: 'relative', zIndex: 2, willChange: 'transform' }}
      >
        <p style={{ fontSize: '12px', letterSpacing: '0.3em', textTransform: 'uppercase', color: 'var(--color-ink, #34281B)', margin: isMobile ? '0 0 18px' : 0, fontWeight: 500 }}>
          {HERO.eyebrow}
        </p>

        <h1 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 700, fontSize: isMobile ? 'clamp(1.9rem,8vw,2.3rem)' : 'clamp(2.4rem,6vw,4.2rem)', lineHeight: 1.12, margin: isMobile ? '0 0 26px' : '18px 0 22px', color: 'var(--color-ink, #34281B)', display: 'flex', flexWrap: 'wrap', alignItems: 'baseline', columnGap: '0.26em', rowGap: '0.05em' }}>
        ...
        {reducedMotion ? (
          <p style={{ display: 'flex', alignItems: 'center', gap: '12px', margin: isMobile ? '0 0 34px' : '18px 0 26px', flexWrap: 'wrap' }}>
          ...
          <>
            <p aria-hidden="true" style={{ display: 'flex', alignItems: 'center', gap: '12px', margin: isMobile ? '0 0 34px' : '18px 0 26px', flexWrap: 'wrap' }}>
